...
export class ConsultaCobrancaComponent implements OnInit, OnDestroy {
  ...
  onDetalhe(cobranca: ConsultaCobranca) {
    //nulo, undefined ou string vazia
    if (!cobranca.tokenAcesso) {
      // neste momento, somente pix automático não possui token de acesso
      console.log('Detalhe desabilitado temporariamente quando não há informações suficientes para consulta');
      return false;
    }
    this.bottomSheetService.create({
