...
export class BoletagemConsultaService {
  private exibirDetalhes: boolean = false;
  ...
  exportarExcelAssincrono(filter, tipoCondicao?: Campo): Observable<ApiResponse> {
    const dataInicio = DateFormatHelper.toUrlDate(filter.dataReferenciaInicio);
    const dataFim = DateFormatHelper.toUrlDate(filter.dataReferenciaFim);

    const path = `boleto/consulta/excel-assincrono/${dataInicio}/${dataFim}/${filter.situacaoTransacao}`;
    const body = FilterConditionBuilder.adicionarFiltro(new BoletagemConsultaFiltro(filter)).build();
    const options = { observe: 'response', responseType: 'blob' };
    ...
    return this.httpClientService.core.post<ApiResponse>({ path, body, options });
  }
  ...
  listarMensagem(params?: { limit?: number, page?: number, sortBy?: string, sortDirection?: Ordenacao },
    filter?: BoletagemConsultaFiltro, tipoCondicao?: Campo): Observable<ListResult<BoletagemConsulta>> {
    const dataInicio = DateFormatHelper.toUrlDate(filter.dataReferenciaInicio);
    const dataFim = DateFormatHelper.toUrlDate(filter.dataReferenciaFim);
    const path = `boleto/listar/${dataInicio}/${dataFim}/${filter.situacaoTransacao}`;
    ...
    return this.httpClientService.pix.post<ListResult<BoletagemConsulta>>({
      path, queryObj, body, responseType: listResult(BoletagemConsulta)
    });
  }
  ...
  getExibirDetalhes(): boolean {
    return this.exibirDetalhes;
  }

  // Atualiza o estado
  setExibirDetalhes(valor: boolean): void {
    this.exibirDetalhes = valor;
  }
}
